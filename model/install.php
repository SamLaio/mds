...
class ModeInstall extends LibDataBase {
	...
	public function St1($arr){
		...
		if($this->dbtype == 'mysql'){
			...
			$this->Query("CREATE TABLE `site` (	`account` TEXT NOT NULL, `url` TEXT NOT NULL, `url` TEXT NOT NULL)COLLATE='latin1_swedish_ci' ENGINE=MyISAM;");
			...
			$this->Query("CREATE TABLE [site] ([name] text  NOT NULL,[url] text NOT NULL,[lang] text NOT NULL);");
		}
		$this->Query($this->In('user', array('account','pswd','name'), array("'".$arr['AdName']."'","'".md5($arr['AdPw'])."'","'Admin'")));
		echo $this->In('site', array('name','url','lang'), array("'".$arr['SiteName']."'","'".$arr['SiteUrl']."'","'".$arr['SiteLang']."'"));
